...
int splashCost(Point pt, NodeCost costAt)
{
    int cost = 0;
    for (int i = -1; i < 2; ++i) {
        for (int j = -1; j < 2; ++j) {
...
Point cheapestPointNear(Point pt, NodeCost costAt)
{
    Point best = pt;
...
findPath(Point from, Point to, NodeCost costAt, BoundsCheck isValid)
{
    ...
    if (dist < 400) {
        return aStarSearch(to, from, costAt, isValid);
    }
    Point mid = cheapestPointNear(
    ...
    joinPath(path, segment.begin(), segment.end());
    return path;
}
